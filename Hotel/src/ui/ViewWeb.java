...
public class ViewWeb extends JPanel {
  ...
  private void mostrarFormularioCargaHuesped() {
    ...
    if (opcion == JOptionPane.OK_OPTION) {
      ...
      // Llamar al método para guardar el huésped (Aquí deberías llamar a tu lógica
      // correspondiente)
      guardarHuesped(nuevoHuesped);
    }
  }

  // Método para mostrar el formulario de carga de habitación - Agregado el método
  // completo
  private void mostrarFormularioCargaHabitacion() {
    ...
    if (opcion == JOptionPane.OK_OPTION) {
      ...
      String tipoStr = comboTipo.getSelectedItem().toString().toLowerCase();
      IHabitacionTipo tipo;
      switch (tipoStr) {
        case "suite":
          tipo = new Suite();
          break;
      ...
      Habitacion nuevaHabitacion = facade.cargarHabitacion(identificador, tipo, capacidad, tarifa, balcon,
          descripcion);
    }
  }
  ...
  private void reservarHabitacion() {
    ...
    if (opcion == JOptionPane.OK_OPTION) {
      try {
        ...
        // Obtener fechas de reserva (aquí debes implementar la lógica para obtener las
        // fechas)
